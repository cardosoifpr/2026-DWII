...
/**
 * Registra uma operacao na trilha de auditoria (tabela logs).
 * O usuario_login vem da sessao, NUNCA do formulario.
 */
...
// -- Processamento do POST (padrao PRG) ----------------------------

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $acao = $_POST['acao'] ?? '';

    // Salvar: id=0 -> cadastro (INSERT); id>0 -> edicao (UPDATE).
    if ($acao === 'salvar') {
        $id          = (int) ($_POST['id'] ?? 0);
        ...
        if ($nome === '' || $descricao === '' || $tecnologias === '') {
            ...
            $link = $link_github === '' ? null : $link_github;
        ...
        // Validacao falhou: preserva o que o usuario digitou.
        $em_edicao = [
            'id'            => $id,
            'nome'          => $nome,
            'descricao'     => $descricao,
    ...
    // Arquivar: soft delete (UPDATE de status), NUNCA DELETE.
    if ($acao === 'arquivar') {
        $id = (int) ($_POST['id'] ?? 0);
...
// -- Listagem com filtro de status (?filtro=...) ------------------
        ...
        <form action="admin.php" method="post" class="formulario">
            ...
            <div class="campo">
                ...
                <textarea name="descricao"
                          class="input-texto"
                          rows="4"><?php echo htmlspecialchars($em_edicao['descricao'] ?? ''); ?></textarea>
            </div>
